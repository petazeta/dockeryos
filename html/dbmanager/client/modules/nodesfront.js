...
const NodeFrontMixing=Sup => class extends Sup {
  ...
  async request(action, parameters, reduce=false, url) {
    const {reqReduc} = await import('./reqs.js');
    let reqRedFuncs=reqReduc.get(action);
    
    if (!Array.isArray(reqRedFuncs)) reqRedFuncs=[reqRedFuncs];
    let nodeRedFunc=reqRedFuncs[0];
    if (!nodeRedFunc) nodeRedFunc=(myNode)=>myNode.cloneNode();
    let paramRedFunc=reqRedFuncs[1];
    if (!paramRedFunc) paramRedFunc=(params)=>params;
    
    const nodeData=nodeRedFunc(this);
    nodeData.avoidrecursion();
    if (parameters) paramRedFunc(parameters);
  ...
  async loadRequest(action, parameters, url) {
    ...
    const {reqLoaders} = await import('./reqs.js');
