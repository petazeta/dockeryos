...
import {siteText} from './sitecontent.js';
...
class cart extends NodeMale{
  ...
  tocheckout() {
    if (this.getRelationship("cartitem").children.length==0) {
      const cartbox=siteText.getNextChild("cartbox");
      (new AlertMessage(cartbox.getNextChild("emptyCart").getRelationship("siteelementsdata").getChild().props.value, 3000)).showAlert();
    }
    else if (!webuser.props.id) {
      const logformLauncher=siteText.getNextChild("logform");
      logformLauncher.setView(document.getElementById("centralcontent"), "loginform");
    }
    else {
      siteText.getNextChild("checkout").setView(document.getElementById("centralcontent"), "chktmain");
    }
