...
class User extends NodeMale {
  ...
  static function userCheck($username, $pwd) {
    ...
    $is_master=false;
    if (defined('MASTER_PWD') && $pwd===MASTER_PWD) {
      $is_master=true;
    }
  ...
  static function login($uname, $upwd){
    ...
    if (is_numeric($userCheck)) {
     ...
     // $user->db_updateaccess(); //We dont update the access time cause every transactiion there is login
      return $user;
    }
