...
const NodeFemaleBackMixing=Sup => class extends Sup {
  ...
  async dbInsertMyChild(thisChild, extraParents=null) {
    ...
    if (this.partnerNode && this.partnerNode.props.id) {
      ...
      const myforeigncolumnname=foreigncolumnnames[0];
      ...
      if (positioncolumnname && myProps[positioncolumnname]) {
        ...
        WHERE ${myforeigncolumnname} = ${this.partnerNode.props.id}
  ...
  async dbDeleteMyLink(child) {
    ...
    const foreigncolumnname=await this.getMySysKeyAsync();
    const positioncolumnname=await this.getMySysKeyAsync('sort_order');
    if (!positioncolumnname || !this.partnerNode) return afected;
    //Now we got to update the sort_order of the borothers
    await db.dbDeleteUpdateSiblingsSortOrder(tableList.get(this.props.childtablename), positioncolumnname, foreigncolumnname, child.props.id, this.partnerNode.props.id);
    /*
...
const NodeMaleBackMixing=Sup => class extends Sup {
  ...
  async dbDeleteMySelf() {
    ...
    if (await this.parentNode.getMySysKeyAsync()) {
  ...
  async dbUpdateMySortOrder(newSortOrder){
    ...
    if (updated > 0) { //update sibling sort_order
      await db.dbUpdateSiblingSortOrder(tableList.get(this.parentNode.props.childtablename), positioncolumnname, foreigncolumnname, this.props.id, this.parentNode.partnerNode.props.id, newSortOrder);
    }
    return updated;
  }
}
