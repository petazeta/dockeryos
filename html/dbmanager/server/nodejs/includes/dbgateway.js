...
const dbConfig=require('./../cfg/dbcfg.json');
...
export function dbUpdateSiblingSortOrder(tableName, positioncolumnname, foreigncolumnname, thisId, foreignId, newOrder){
  let sql;
  if (dbConfig.dbsys=="mysql") {    
    ...
    WHERE dt1.id != ${thisId}
    AND dt1.${foreigncolumnname} = ${foreignId}
...
export function dbDeleteUpdateSiblingsSortOrder(tableName, positioncolumnname, foreigncolumnname, thisId, foreignId){
  let sql;
  if (dbConfig.dbsys=="mysql") {    
    sql = `UPDATE ${tableName} dt1
    INNER JOIN ${tableName} dt2
    ...
    WHERE dt1.${foreigncolumnname} = ${foreignId}
